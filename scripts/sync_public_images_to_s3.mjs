...
async function uploadToS3(filePath) {
  ...
  // Nén WebP nếu là JPG / PNG
  let finalBuffer = buffer;
  let finalKey = relPath;
  ...
  console.log(`✅ [${Math.round(finalBuffer.length / 1024)} KB] ${finalKey} -> ${PUBLIC_URL}/${finalKey}`);
}

async function startSync() {
  console.log('🚀 Bắt đầu quét và đồng bộ kho ảnh từ public/images lên S3...');
  const imagesDir = path.resolve('public/images');
  const allImages = getAllImageFiles(imagesDir);

  console.log(`Tìm thấy ${allImages.length} hình ảnh cần đồng bộ.`);
  ...
  console.log(`\n🎉 ĐÃ ĐỒNG BỘ THÀNH CÔNG ${count} ẢNH LÊN S3!`);
}

...
export const ThatPhatDuocSuSection: FC = () => {
  const [selectedStatue, setSelectedStatue] = useState<ThatPhatDuocSuItem | null>(null);
  ...
  return (
    <div className="w-full py-8 md:py-12 relative overflow-hidden bg-transparent">
      ...
      <div className="flex flex-col items-center text-center mb-8">
        ...
        >
          7 Hóa Thân Đức Phật Dược Sư • Bổn Nguyện Cứu Độ &amp; Tiêu Trừ Nghiệp Chướng Khổ Đau
        </p>
      </div>

      {/* ── 7 STATUES CAROUSEL / ROW GRID ── */}
      <div className="w-full max-w-[1400px] mx-auto px-2 sm:px-4">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-3 sm:gap-4 md:gap-4.5">
          {THAT_PHAT_DUOC_SU_DATA.map((item, idx) => (
            <div
              key={item.id || idx}
              onClick={() => setSelectedStatue(item)}
              className="group relative rounded-2xl border border-[#F2C14E]/35 bg-[#25170E] hover:border-[#F2C14E] transition-all duration-300 shadow-xl cursor-pointer overflow-hidden flex flex-col justify-between h-[340px] sm:h-[370px] transform-gpu hover:-translate-y-1.5 hover:shadow-[0_12px_30px_rgba(242,193,78,0.25)]"
            >
              {/* IMAGE FRAME */}
              <div className="relative w-full h-[260px] sm:h-[285px] overflow-hidden bg-[#1A120B] shrink-0">
                <img
                  src={item.imgUrl}
                  alt={item.name}
                  loading="lazy"
                  decoding="async"
                  className="w-full h-full object-cover group-hover:scale-108 transition-transform duration-700 transform-gpu filter drop-shadow-[0_6px_16px_rgba(0,0,0,0.8)]"
                  onError={(e) => {
              ...
              <div className="absolute inset-x-0 bottom-0 z-20 transition-transform duration-500 ease-out translate-y-[28px] group-hover:translate-y-0 transform-gpu">
                ...
                {/* Content Box */}
                <div className="relative w-full bg-gradient-to-b from-[#25170E] to-[#1C130D] px-2.5 pt-4 pb-3 text-center flex flex-col items-center justify-start">
                  ...
                  {/* Title & Subtitle */}
                  <div className="w-full flex flex-col items-center mt-1 shrink-0">
                    <h3
                      className="text-[#F2C14E] text-lg sm:text-xl font-normal tracking-wide uppercase group-hover:text-white transition-colors line-clamp-1 mb-0.5"
                    ...
                    <p
                      className="text-[#FFE5A3]/90 text-[10px] font-bold line-clamp-1"
                      style={{ fontFamily: "'UTM Avo', sans-serif" }}
                    >
                      {item.worldName || item.subtitle}
                    </p>
                  </div>
              ...
              <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-start">
                ...
                <div className="w-full md:w-[60%] flex-1 min-w-0 text-left space-y-4">
                  ...
                  <p
                    ...
                    style={{ fontFamily: "'UTM Avo', sans-serif" }}
                  >
                    {selectedStatue.subtitle}
                  </p>

                  {/* KHU VỰC VÀ THẾ GIỚI TÔN THỜ */}
                  <div className="space-y-1.5 bg-[#1C130D]/80 p-3.5 rounded-xl border border-[#F2C14E]/30 text-xs text-[#FFE5A3]">
                    <p><span className="text-[#F2C14E] font-bold">Cõi Nước:</span> {selectedStatue.worldName}</p>
                    <p><span className="text-[#F2C14E] font-bold">Khu Vực Phối Thờ:</span> {selectedStatue.areaName || 'Khu Vực Tam Bảo (Chánh Điện)'}</p>
                  </div>
                  ...
                  {selectedStatue.quote && (
                    <div
                      ...
                      style={{ fontFamily: "'UTM Avo', sans-serif" }}
                    >
                      <span className="not-italic font-bold text-[#F2C14E] block mb-1">Lời Dạy &amp; Bổn Nguyện:</span>
